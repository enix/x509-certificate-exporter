...
func New(opts Options, logger *slog.Logger) *Source {
	...
	return &Source{
		opts:   opts,
		log:    logger.With("source_kind", "kubeconfig", "source_name", opts.Name),
		parser: pem.New(),
...
func (s *Source) scan(path string, sink cert.Sink, seen map[string]struct{}, allOK *bool) {
	...
	if err != nil {
		*allOK = false
		...
		ref := cert.SourceRef{Kind: "kubeconfig", Location: path, SourceName: s.opts.Name, Format: "pem"}
