...
| prometheusRules.alertOnKubeTransportErrors | bool | `true` | Should the KubeTransportErrors / KubeTransportErrorsSustained alerting rules be created to notify when the Kubernetes source records LIST/WATCH/informer-level failures (auth rotation drift, apiserver throttling, persistent network issues). The two share one toggle and one expression but two severities: warning fires after >5 errors in 15 minutes (filters out the 1–3 events a rolling control-plane upgrade typically produces), critical fires once that condition has held continuously for 30 minutes (sustained dysfunction, not a transient blip). Independent from X509ExporterReadErrors: the latter covers per-HTTP-response errors via `x509_source_errors_total`, this one covers higher-level transport state (watch flapping, list backoff, namespace informer sync timeouts). |
