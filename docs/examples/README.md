...
| **TKGS / vSphere with Tanzu** (VMware) | [`distros/tkgs.values.yaml`](./distros/tkgs.values.yaml) | Workload/Guest cluster (TKC) PKI — identical to the kubeadm layout since TKC nodes are provisioned via CABPK (Cluster API Bootstrap Provider Kubeadm). Verified on Kubernetes v1.33.3+vmware.1-fips / Photon OS 5.0; also includes `kubelet.conf` in the control-plane kubeconfig list, unlike the vanilla kubeadm example. Does **not** cover the Supervisor cluster, which uses the proprietary CAPW provider and has no node-level access. |
